import{getGraphQLQueryArgs, getGraphQLFilterType} from 'graphql-to-mongodb'
...
export abstract class SchemaGenerator {
  ...
  private static buildRootQueryType(): GraphQLObjectType {
    ...
    Object.values(a.getFields()).forEach((f:any) => {
      Object.values(f.type.ofType.ofType.getFields()).forEach((d:any) => d.resolve = undefined)
      const args:any = getGraphQLQueryArgs(f.type.ofType.ofType)
      f.args = Object.keys(args).map(k => {const a = args[k];a.name = k;return a;})
    })
    return a;
  }
